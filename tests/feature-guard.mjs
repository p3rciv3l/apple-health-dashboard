...
check('Workout burn included in blow-up focus stack + tooltip (Owner 8/26: blow-up view exactly pre-change)', /focusBar\(di, i, segs\)/.test(appJs) && !/segs\.filter\(s => !s\.wo\)/.test(appJs), 'workout band missing from the blow-up in-chart stack/tooltip');
check('Blow-up focus stack re-applied after re-render (Owner 8/26: resize wiped the stack, "no shading on the green bar")', /eng\.focus && eng\.focus\.segs\) \{ const f = eng\.focus; focusBar\(f\.di, f\.i, f\.segs\)/.test(appJs) && /prevDisplay: el\.style\.display, segs: segs/.test(appJs), 'focus stack is not restored after a resize re-render');
